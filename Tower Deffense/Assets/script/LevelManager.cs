...
{ 
    ...
    [SerializeField] private screenPause gamePause;

    private void Start()
    {
        //menampilkan tombol pause disetiap awal permainan
        btnpause.SetActive(true);
        //memulai suara latar belakan setiap mulai permainan
        bgsound.Play();
        bgsound.loop = true;
    ...
    private void Update()
    {
        Bullet pausebulet = new Bullet();
        pausebulet.gamepause = pausegame;
        // Jika menekan tombol R, fungsi restart akan terpanggil
        if (Input.GetKeyDown(KeyCode.R))
        {
            SceneManager.LoadScene(SceneManager.GetActiveScene().name);
        }
        if (pausegame == false)/*memeanggil fungsi pause*/
        {
            if (IsOver)
            {
                return;
    ...
    {
        IsOver = true;
        //menghilangkan tombol pause saat gameover atau menang
